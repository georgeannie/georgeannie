...
dependent_layout_row1 = fluidRow( 
    ...
    column(width=10, 
            conditionalPanel(
               condition="input.taba.radio_dep_choice == 'Histogram'",
                 plotOutput("hist_dependent", height=400, width=970)),
                                  
            conditionalPanel(
              condition="input.taba.radio_dep_choice == 'Correlation matrix'",
                column(width=5, div(style = "height:50px;background-color: black;", "Test")),
                column(width=5, div(style = "height:50px;background-color: black;")) )
    )
)

            #  column(width=2, div(style = "height:50px;background-color: black;",))
            #           hist_dep),
